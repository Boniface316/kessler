...
from ..io.schemas import InputsSchema, Inputs
...
class DummyJob(Job):
    ...
    KIND: T.Literal["DummyJob"] = "DummyJob"
    ...
    def run(self) -> Locals:
        ...
        with self.mlflow_service.run_context(run_config=self.run_config) as run:
            ...
            inputs_ = self.inputs.read(columns_to_keep)  # unchecked!
            ...
            # - index
            train_index, test_index = next(self.splitter.split(inputs=inputs))
            # # - inputs
