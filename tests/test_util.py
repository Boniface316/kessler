...
class UtilTestCase(unittest.TestCase):
    def test_tle(self):
        ...
        line1, line2 = original.util.tle(**args)
        line1Correct = (
        ...
        line1, line2 = original.util.tle(**args)
        line1Correct = (
            "1 43437U 18100A   20143.90384230  .00009660  17108+9 -10000-2 0 99966"
        )
        line2Correct = (
            "2 43437  86.4364 221.8435 0274000 285.0866 348.2151 13.75098708563531"
        )
    ...
    def test_from_datetime_to_cdm_datetime_str(self):
        ...
        date_str = original.util.from_datetime_to_cdm_datetime_str(date)
    ...
    def test_from_jd_to_cdm_datetime_str(self):
        ...
        date_str = original.util.from_jd_to_cdm_datetime_str(jd_date)
    ...
    def test_from_TEME_to_ITRF(self):
        state_TEME = np.array(
            [
                [5094.18016210 * 1e3, 6127.64465950 * 1e3, 6380.34453270 * 1e3],
                [-4.746131487 * 1e3, 0.785818041 * 1e3, 5.531931288 * 1e3],
            ]
        )

        jd_date = 2433282.4235
        state_ITRF = original.util.from_TEME_to_ITRF(state_TEME, jd_date)
        state_ITRF_correct = np.array(
            [
                np.array([7377976.66089733, -3010674.50719708, 6380344.5327]),
                np.array([-900.58420598, 4224.28457883, 5531.931288]),
            ]
        )
        ...
        self.assertAlmostEqual(
            np.linalg.norm(state_TEME[0]), np.linalg.norm(state_ITRF[0]), places=1
        )

    def test_from_cartesian_to_rtn(self):
        state_xyz = np.array([[1.0, 2.0, 3.4], [4.5, 6.2, 7.4]])
        state_rtn, cartesian_to_rtn_rotation_matrix = (
            original.util.from_cartesian_to_rtn(state_xyz)
        )
        rtn_to_cartesian_rotation_matrix = cartesian_to_rtn_rotation_matrix.T
        r_xyz, v_xyz = (
            np.matmul(rtn_to_cartesian_rotation_matrix, state_rtn[0]),
            np.matmul(rtn_to_cartesian_rotation_matrix, state_rtn[1]),
        )

        self.assertTrue(np.allclose(state_xyz[0], r_xyz))
        self.assertTrue(np.allclose(state_xyz[1], v_xyz))
        self.assertAlmostEqual(
            np.linalg.norm(state_xyz[0]), np.linalg.norm(state_rtn[0]), places=1
        )
        self.assertAlmostEqual(
            np.linalg.norm(state_xyz[1]), np.linalg.norm(state_rtn[1]), places=1
        )

    def test_from_cartesian_to_rtn_2(self):
        state_xyz = np.array([[1.0, 2.0, 3.4], [4.5, 6.2, 7.4]])
        state_rtn, _ = original.util.from_cartesian_to_rtn(state_xyz)
        self.assertAlmostEqual(
            np.linalg.norm(state_xyz[0]), np.linalg.norm(state_rtn[0]), places=1
        )
        self.assertAlmostEqual(
    ...
    def test_get_ccsds_time_format(self):
        ...
        self.assertEqual(
            original.util.get_ccsds_time_format(test_case1), test_case1_correct
        )
        self.assertEqual(
            original.util.get_ccsds_time_format(test_case2), test_case2_correct
        )
    ...
    def test_doy_2_date(self):
        ...
        example4 = (
            "2010-001T00:00:00.000"  # Check that works at the first day of a year
        )
        example5 = (
            "2012-366T00:00:00.000"  # Check that works at the final day of a leap year
        )
        ...
        self.assertEqual(
            original.util.doy_2_date(example1, doy_1, year_1, 5), test_case1_correct
        )
        self.assertEqual(
            original.util.doy_2_date(example2, doy_2, year_2, 5), test_case2_correct
        )
        self.assertEqual(
            original.util.doy_2_date(example3, doy_3, year_3, 5), test_case3_correct
        )
        self.assertEqual(
            original.util.doy_2_date(example4, doy_4, year_4, 5), test_case4_correct
        )
        self.assertEqual(
            original.util.doy_2_date(example5, doy_5, year_5, 5), test_case5_correct
        )
